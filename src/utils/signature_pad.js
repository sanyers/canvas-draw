...
class SignaturePad extends SignatureEventTarget {
  constructor(canvas, options = {}) {
    ...
    this.drawType = options.drawType || 'pencil';
    this.cancelHistory = [];
  ...
  _strokeUpdate(event, isFirst = false) {
  ...
  _strokeEnd(event) {
    this._strokeUpdate(event, false, true);
  ...
  _drawArc(point, options) {
    ...
    ctx.arc(startPoint.x, startPoint.y, r, 0, Math.PI * 2, false);
    ctx.strokeStyle = options.penColor;
    ctx.closePath();
    ctx.stroke();
  }
  ...
  draw() {
    if (this.optionsList) {
      this.dotSize = this.optionsList.dotSize;
      this.minWidth = this.optionsList.minWidth;
      this.maxWidth = this.optionsList.maxWidth;
      this.penColor = this.optionsList.penColor;
    }
  }
  // 撤回
  cancel() {
    ...
    const last = data.pop();
    if (last) {
      this.cancelHistory.push(last);
      this.fromData(data);
    }
  }
  // 前进
  go() {
    if (this.cancelHistory.length) {
      const data = this.toData();
      const last = this.cancelHistory.pop();
      if (last) {
        this.fromData([...data, last]);
      }
    }
  }
  getImageDataNow() {
